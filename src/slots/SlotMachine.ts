...
export class SlotMachine {
  ...
  public update(delta: number): void {
    ...
    if (this.waitingForLastReel && lastReel.hasFullyStopped) {
      this.waitingForLastReel = false;
  ...
  private checkWin(): void {
    ...
    if (randomWin) {
      ...
      if (this.winAnimation) {
        ...
        const entry = this.winAnimation.state.setAnimation(0, "start", false);
